defmodule ConveyorBackendWeb.AuthCookies do
  @moduledoc """
  Single source of truth for the two authentication cookies.

    * `"conveyor_token"` — the short-lived session JWT. Deliberately set with
      **no `:max_age`**, so it is a browser-session cookie: it dies when the
      browser closes. That *is* the "remember me unchecked" behaviour.
    * the remember-me cookie — its name and lifetime come from the RememberMe
      strategy DSL (`:conveyor_remember_me`, 30 days).

  Both are HTTP-only and `SameSite=Lax`: JavaScript never sees a token.
  """

  import Plug.Conn

  @auth_cookie "conveyor_token"

  def auth_cookie_name, do: @auth_cookie

  def put_auth_cookie(conn, token), do: put_resp_cookie(conn, @auth_cookie, token, base_opts())

  def put_remember_me_cookie(conn, name, token, max_age),
    do: put_resp_cookie(conn, to_string(name), token, [max_age: max_age] ++ base_opts())

  def delete_cookie(conn, name), do: delete_resp_cookie(conn, to_string(name), path: "/")

  defp base_opts do
    [
      http_only: true,
      secure: Application.get_env(:conveyor_backend, :secure_cookies, true),
      same_site: "Lax",
      path: "/"
    ]
  end
end
