defmodule ConveyorBackend.Accounts.UserTest do
  @moduledoc """
  Action and policy tests for `ConveyorBackend.Accounts.User`.
  """
  ...
  alias ConveyorBackend.Accounts
  alias ConveyorBackend.Accounts.Token
  ...
  describe "register_with_password" do
    ...
    test "issues a remember-me token when requested" do
      ...
      assert %{token: token, cookie_name: :conveyor_remember_me, max_age: 2_592_000} =
  ...
  describe "sign_in_with_password" do
    ...
    test "issues a remember-me token when requested" do
      ...
      assert %{cookie_name: :conveyor_remember_me, token: token} = user.__metadata__.remember_me
      assert is_binary(token)
    end
  end
